...
CREATE TABLE users (
    ...
    profile_file_id BIGINT NULL COMMENT '프로필 이미지 파일ID',
...
CREATE TABLE board_drafts (
    ...
    INDEX `idx_boards_draft_user_id` (user_id),
    INDEX `idx_boards_draft_updated_at` (updated_at),
    
    CONSTRAINT `fk_board_draft_user` FOREIGN KEY (user_id) REFERENCES users(id)
)	ENGINE InnoDB
	DEFAULT CHARSET = utf8mb4
    COLLATE = utf8mb4_unicode_ci
    COMMENT = '게시글 임시저장 테이블';
